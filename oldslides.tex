...
\begin{frame}
  \frametitle{Efficient computation of optimal last segment cost}
For any $0\leq i < j \leq n$ we have    
\begin{eqnarray*}
  c_{(i, j]}
%  &=&  \min_u \sum_{t=i+1}^{j} \ell( u,  z_t)\\
  &=&  \min_u \sum_{t=i+1}^{j} ( u - z_t)^2\\
  &=&  \min_u \sum_{t=i+1}^{j} ( u^2 -2 u z_t + z_t^2)\\
  &=&  \sum_{t=i+1}^{j}(z_t^2) + \min_u (j-i) u^2 -2 u \sum_{t=i+1}^{j} (z_t)   \\
%  &=&  Q_j - Q_i + (j-i) [(S_j-S_i)/(j-i)]^2 -2 [(S_j-S_i)/(j-i)] (S_j-S_i) \\
  &=&  Q_j - Q_i + (S_j-S_i)^2/(j-i) -2 (S_j-S_i)^2/(j-i) \\
  &=&  Q_j - Q_i - (S_j-S_i)^2/(j-i) 
\end{eqnarray*}
where
\begin{itemize}  
\item $Q_0=0$ and $Q_i=Q_{i-1} + z_i^2$ for all $i\in\{1, \dots, n\}$ (cumulative sum of squares of data).
\item $S_0=0$ and $S_i=S_{i-1} + z_i$ for all $i\in\{1, \dots, n\}$ (cumulative sum of data).
\end{itemize}
\end{frame} 
